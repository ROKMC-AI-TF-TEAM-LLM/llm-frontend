...
- **`useServerStatus.ts`**: 헬스 폴링 → `'checking'|'ok'|'error'`. db+llm_server 둘 다 OK여야 ok, 아니면 10초 간격 재시도.
...
| 분류 | 경로 | 역할 | 주요 export |
|------|------|------|------------|
| 진입점 | `src/main.tsx` | 앱 부팅, Provider 조립, 청크 에러 핸들러 | — |
| 진입점 | `src/App.tsx` | 라우터 정의 (공개/보호/관리자 라우트 트리) | `App` |
| API 통신 | `src/api/lib/axios.ts` | Axios 인스턴스 + JWT 자동 갱신 (핵심) | `backendApi`, `getValidAccessToken`, `scheduleTokenRefresh` |
| API 통신 | `src/api/queryClient.ts` | React Query 전역 클라이언트 | `queryClient` |
| API 통신 | `src/api/services/auth.ts` | 로그인/회원가입/로그아웃/토큰갱신 | `login`, `signup`, `logout`, `refresh` |
| API 통신 | `src/api/services/session.ts` | 세션 CRUD + 커서 페이지네이션 | `getSessions`, `createSession`, `searchSessions` 등 |
| API 통신 | `src/api/services/chat.ts` | SSE 스트리밍 (fetch 기반) + 메시지 조회/삭제 | `streamMessage`, `regenerateMessageStream`, `getMessages` |
| API 통신 | `src/api/services/document.ts` | 문서 목록 조회 (30s 타임아웃, 응답 정규화) | `getDocuments`, `pickDocuments` |
| API 통신 | `src/api/services/user.ts` | 내 정보 + 관리자 유저 관리 | `getMeUsers`, `getUsers`, `approveUser` 등 |
| API 통신 | `src/api/services/inquiry.ts` | 문의 CRUD + 관리자 답변 | `getInquiries`, `createInquiry` 등 |
| API 통신 | `src/api/services/health.ts` | 서버 상태 조회 | `getHealth` |
| 상태관리 | `src/api/store/chatStore.ts` | 채팅 스트리밍 Zustand 스토어 (최대·핵심 파일) | `useChatStore` |
| 상태관리 | `src/context/AuthContext.tsx` | 인증 Context + 토큰 관리 | `AuthProvider`, `useAuth` |
| 상수 | `src/constants/key.ts` | 스토리지 키 상수 | `LOCAL_STORAGE_KEY` |
| 유틸 | `src/utils/error.ts` | API 에러 → 사용자 메시지 변환 | `getApiError`, `isNetworkError`, `DEFAULT_STATUS_ERRORS` |
| 유틸 | `src/utils/logError.ts` | 일관 콘솔 에러 로깅 | `logError` |
| 유틸 | `src/utils/chunkReload.ts` | 청크 로드 실패 시 1회 자동 새로고침 | `tryReloadOnce` |
| 훅 | `src/hooks/useSession.ts` | 세션 무한스크롤 + CRUD React Query 훅 | `useInfiniteSessions`, `useCreateSession` 등 |
| 훅 | `src/hooks/useUser.ts` | 내 정보 + 관리자 유저 React Query 훅 | `useGetMe`, `useInfiniteUsers` 등 |
| 훅 | `src/hooks/useDocument.ts` | 문서 무한스크롤 | `useInfiniteDocuments` |
| 훅 | `src/hooks/useServerStatus.ts` | 서버 헬스 폴링 (10초 간격) | `useServerStatus` |
| 훅 | `src/hooks/useLocalStorage.ts` | localStorage/sessionStorage get/set/remove | `useLocalStorage` |
| 훅 | `src/hooks/useDocumentTitle.ts` | 페이지 타이틀 설정 + 언마운트 복원 | `useDocumentTitle` |
| 타입 | `src/types/index.ts` | 프론트 도메인 모델 (Message 유니온 등) | `Message`, `UserMessage`, `AssistantMessage`, `Source` 등 |
| 타입 | `src/types/chat.ts` | 채팅 API DTO (human/ai role) | `MessageItem` |
| 타입 | `src/types/auth.ts` | 인증 요청/응답 DTO | `LoginRequest`, `AuthResponse` |
| 타입 | `src/types/session.ts` | 세션 DTO | `Session`, `SessionsResponse` |
| 타입 | `src/types/user.ts` | 유저 DTO | `User`, `UsersResponse` |
| 타입 | `src/types/document.ts` | 문서 DTO | `RagDocument` |
| 타입 | `src/types/inquiry.ts` | 문의 DTO | `Inquiry` |
| 타입 | `src/types/api.ts` | 공통 응답 래퍼 | `ApiResponse` |
| 페이지 | `src/pages/LoginPage.tsx` | 랜딩 인트로 + 로그인/회원가입 패널 | `LoginPage` |
| 페이지 | `src/pages/ChatPage.tsx` | 채팅 화면 (`/chat/:id`) | `ChatPage` |
| 페이지 | `src/pages/NewChatPage.tsx` | 새 채팅 시작 (`/chat`) | `NewChatPage` |
| 페이지 | `src/pages/SearchPage.tsx` | 세션 검색 (lazy) | `SearchPage` |
| 페이지 | `src/pages/RagPage.tsx` | 문서 목록 (lazy) | `RagPage` |
| 페이지 | `src/pages/AdminPage.tsx` | 관리자 유저 관리 (lazy) | `AdminPage` |
| 페이지 | `src/pages/ErrorPage.tsx` | 404/라우트 에러 폴백 | `ErrorPage` |
| 레이아웃 | `src/ui/layouts/AuthLayout.tsx` | 로그인 가드 (이미 로그인이면 /chat 리다이렉트) | `AuthLayout` |
| 레이아웃 | `src/ui/layouts/ProtectedLayout.tsx` | 인증 가드 + 사이드바 셸 | `ProtectedLayout` |
| 레이아웃 | `src/ui/layouts/AdminLayout.tsx` | 관리자 권한 가드 | `AdminLayout` |
| 컴포넌트 | `src/ui/components/MarsPlanet.tsx` | 순수 CSS 화성 행성 (로그인·AI 아바타) | `MarsPlanet` |
| 컴포넌트 | `src/ui/components/Toast.tsx` | 5초 자동 소멸 토스트 (error/success) | `Toast` |
| 컴포넌트 | `src/ui/components/Skeleton.tsx` | 로딩 스켈레톤 프리셋 모음 | `Skeleton` |
| 컴포넌트 | `src/ui/components/ErrorBoundary.tsx` | 렌더 에러 캐치 폴백 | `ErrorBoundary` |
| 컴포넌트 | `src/ui/components/chat/ChatInput.tsx` | 자동 높이 textarea + 세션 생성 + 이미지 첨부 | `ChatInput` |
| 컴포넌트 | `src/ui/components/messages/MessageList.tsx` | 메시지 목록 + 스크롤 앵커링 + 재생성 버튼 | `MessageList` |
| 컴포넌트 | `src/ui/components/messages/MessageBubble.tsx` | 말풍선 (유저=빨강 그라데이션, AI=흰색+마크다운) | `MessageBubble` |
| 컴포넌트 | `src/ui/components/messages/MessageActions.tsx` | 말풍선 hover 액션 (복사/재생성/시간) | `MessageActions` |
| 컴포넌트 | `src/ui/components/messages/ChatHeader.tsx` | 채팅 상단 세션 제목 | `ChatHeader` |
| 컴포넌트 | `src/ui/components/messages/SourceBadge.tsx` | RAG 출처 문서 토글 뱃지 | `SourceBadge` |
| 컴포넌트 | `src/ui/components/messages/ImageAttachment.tsx` | 이미지 메시지 렌더 | `ImageAttachment` |
| 컴포넌트 | `src/ui/components/sidebar/Sidebar.tsx` | 사이드바 조립 (헤더+메뉴+최근대화+푸터) | `Sidebar` |
| 컴포넌트 | `src/ui/components/sidebar/SidebarHeader.tsx` | 로고 + 접기 토글 | `SidebarHeader` |
| 컴포넌트 | `src/ui/components/sidebar/SidebarMenu.tsx` | 새 채팅/검색 네비 메뉴 | `SidebarMenu` |
| 컴포넌트 | `src/ui/components/sidebar/RecentChats.tsx` | 최근 세션 무한 목록 + 이름 수정/삭제 | `RecentChats` |
| 컴포넌트 | `src/ui/components/sidebar/SidebarFooter.tsx` | 프로필 버튼 → 클로드식 드롭다운 메뉴 | `SidebarFooter` |
| 컴포넌트 | `src/ui/components/rag/RagCard.tsx` | 문서 카드 | `RagCard` |
| 컴포넌트 | `src/ui/components/rag/RagSearchInput.tsx` | 문서 이름 필터 입력 | `RagSearchInput` |
| 컴포넌트 | `src/ui/components/search/SearchInput.tsx` | 세션 검색 입력 (300ms 디바운스) | `SearchInput` |
| 컴포넌트 | `src/ui/components/search/SearchResults.tsx` | 검색 결과 목록 | `SearchResults` |
| 컴포넌트 | `src/ui/components/search/SearchResultItem.tsx` | 검색 결과 항목 + `SearchResult` 타입 | `SearchResultItem`, `SearchResult` |
| 스타일 | `src/ui/styles/global.css` | 디자인 토큰(@theme) + 로그인 화면(.mars-*) + 공통 keyframes | — |
